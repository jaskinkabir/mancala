    ...
    while (true) { //Doesn't move on until player lands on a space that is not their bank
        ...
        case 1: //code for player 1's turn
            ...
            while (true)
            {
                ...
                if (a == 99) { // Demonstrate capture
                    board[2] = 1;
                    board[3] = 0;
                    board[11]++;
                }

                if (a == 101) {//demonstrate checkwin
                    board = { 5,0,0,0,0,0,0,6,0,3,4,0,0,0 };
                    clear();
                    printBoard(selected);
                    Sleep(500);
                    return -1;
                ...
                if (a == 98) board[6] = 1;  //Demonstrate repeat turn
        ...
        case 2: //Code for player 2's turn
            ...
            while (true)
            {
                ...
                if (a == 98) board[13] = 1;  //Demonstrate repeat turn (press b)

                if (a == 101) {//demonstrate checkwin (press e)
                    board = { 5,0,0,0,0,0,0,6,0,3,4,0,0,0 };
                    clear();
                    printBoard(selected);
                    Sleep(500);
        ...
        int marbles = board[selected]; //Set up screen for "animating" turn
        clear();
        printBoard(-1);
        Sleep(500);
        board[selected] = 0;
...
void checkWin() {
    int marbles;
    if (board[13] == 0 && board[12] == 0 && board[11] == 0 && board[10] == 0 && board[9] == 0 && board[8] == 0) { //Check if player 2's side of the board is empty
        cout << "Game over!" << endl;
        Sleep(500);
        for (int i = 1; i < 7; i++) { //loop through player 1's side of the board, adding all of the stones to player 1's bank
            marbles = 0;
            if (board[i] != 0) {
    ...
    if (board[1] == 0 && board[2] == 0 && board[3] == 0 && board[4] == 0 && board[5] == 0 && board[6] == 0) { //Check if player 1's side of the board is empty
        cout << "Game over!" << endl;
        Sleep(500);
        for (int i = 8; i < 14; i++) { //loop through player 2's side of the board, adding all of the stones to player 1's bank
            marbles = 0;
            if (board[i] != 0) {
                marbles = board[i]; //Display board with selected space emptied
                board[i] = 0;
...
/*  Press c to demonstrate capture
    ...
    press e to demonstrate check win
*/